...
// Fetch job details if a job is requested
if (isset($_GET['id'])) {
    $job_id = $_GET['id'];
    ...
    if (isset($_FILES['cover_letter']) && $_FILES['cover_letter']['error'] === 0) {
        ...
        if (in_array(strtolower($file_ext), $allowed_ext)) {
            ...
            if (move_uploaded_file($_FILES['cover_letter']['tmp_name'], $file_path)) {
                // Insert the application details along with the file path into the database, including user_id
                $query = "INSERT INTO job_applications (job_id, user_id, name, email, cover_letter) VALUES (?, ?, ?, ?, ?)";
                $stmt = $pdo->prepare($query);
                $stmt->execute([$job_id, $user_id, $name, $user_email, $file_path]);

                // Redirect to a success page or back to job details with success message
                header("Location: job-details.php?id=$job_id&application=success");
                exit();
            } else {
...
<div class="container my-5">
    <?php if ($job): ?>
        ...
        <p><strong>Posted On:</strong> <?php echo $job['posted_at']; ?></p>
        ...
        <div id="apply-form" style="display: none;">
            ...
            <form action="job-details.php?id=<?php echo $job['id']; ?>" method="POST" enctype="multipart/form-data">
